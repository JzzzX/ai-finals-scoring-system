...
2026-09-18，在上述初始状态验收之后，按项目负责人要求增加 `judge` 初版查看账号，并为管理员及评委设置便于联调的密码，明文列于 README。此时本机为 1 名管理员、1 名查看评委、0 份评分，赛事仍为 `draft`；前文 0 位评委记录和截图为修改前快照。

已通过实际 HTTP 请求验证两账号登录成功、管理员成绩接口可用、评委工作区可用，评委访问管理员成绩接口返回 403。本次调整未开始赛事、未产生评分。调整前已执行一致性数据库备份。

新增 `docs/developer-handoff.md`，并同步修正 README 与飞书接入说明的账号描述。本次文档更新不改变应用代码；检查 Markdown 本地链接和 Git 差异，不重复运行未受影响的应用测试。
