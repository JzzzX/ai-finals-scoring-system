...
首页选择评委姓名并确认后进入评分；右上角「管理员登录」使用原管理员账密。后台新增普通评委只需姓名；正式开赛前配置名单，开赛时固定评分名单。名单为空时首页明确提示，不创建示例评委。

姓名入口不验证本人身份，请现场提醒评委仅选择本人；姓名公开可见。此入口签发的会话只具有评委权限，即使该成员同时拥有管理员角色或以后被提升权限，也不能访问任何管理接口。管理员仍需密码登录。停用成员立即撤销会话。SQLite 自动增加会话权限范围字段，原账户会话兼容；发布前备份数据库。

此功能仅用于 GitHub/Zeabur 外部版，Gitea 内部飞书版保持独立。
